...
- `.fit`: fits a RandomForest with `n_estimators` trees. Uses RandomK feature selection (`m_try` features). Supports reproducible randomness via `seed`.
...
rf = RandomForest(n_estimators = 30, m_try = 5, max_depth = 5, seed = 10)
````
