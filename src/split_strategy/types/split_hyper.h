 ...
 * @param n_estimators Optional number of trees to train in Forest models
 * @param max_depth Optional maximum depth allowed to be reached in training
 * @param max_samples Optional percentage of total samples to be bootstrapped in Forest models 
 * @param min_sample_split Optional minimum number of samples allowed in a leaf
 * @param n_random_split Optional number of random threshold to draw for ExtraTree
...
struct HyperParam{
    ...
    std::optional<int> n_random_split = std::nullopt;
